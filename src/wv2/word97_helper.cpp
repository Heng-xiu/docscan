...
U16 determineParameterLength(U16 sprm, const U8* in, WordVersion version)
{
    if (version == Word8) {
        ...
        if (operandSizes[ index ] != 0) {
            ...
            wvlog << "==> Size of the sprm argument:" << (U16) operandSizes[index] << std::endl;
...
S16 CHP::applyCHPSPRM(const U8* ptr, const Style* paragraphStyle, const StyleSheet* styleSheet, OLEStreamReader* dataStream, WordVersion version)
{
    ...
    case SPRM::sprmCHpsNew50:
        if (*ptr != 2)
            wvlog << "Warning: sprmCHpsNew50 has a different length than 2" << std::endl;
        else
            hps = readU16(ptr + 1);
        break;
    ...
    case SPRM::sprmCFDStrike:
        if (*ptr < 128) {
            ...
            wvlog << "Warning: sprmCFDStrike couldn't find a style" << std::endl;
            fDStrike = !fDStrike;
        }
        break;
    ...
    case SPRM::sprmCFSpec:
        if (*ptr < 128) {
            ...
            wvlog << "Warning: sprmCFSpec couldn't find a style" << std::endl;
            fSpec = !fSpec;
        }
        break;
    case SPRM::sprmCFObj:
    ...
    case SPRM::sprmCDispFldRMark: {
        if (*ptr != 39) {
            wvlog << "Warning: sprmCDispFldRMark has a different length than 39" << std::endl;
        } else {
...
//                         std::bind1st( std::plus<S16>(), dxaNew ) );
        ...
        dxaLeft = readS16(ptr);
#ifdef WV2_DEBUG_SPRMS
        wvlog << "dxaLeft: " << dxaLeft << std::endl;
#endif
        break;
    }
...
//         }
...
#ifdef WV2_DEBUG_SPRMS
        wvlog << "dxaGapHalf: " << dxaGapHalf << std::endl;
#endif
        break;
    }
...
#ifdef WV2_DEBUG_SPRMS
        wvlog << "rgdxaCenter[0]: " << rgdxaCenter[0] << std::endl;
#endif
    ...
    case SPRM::sprmTDefTableShd2nd: {
        wvlog << "Warning: sprmTDefTableShd2nd not implemented" << std::endl;
        break;
    }
    case SPRM::sprmTDefTableShd3rd: {
        wvlog << "Warning: sprmTDefTableShd3rd not implemented" << std::endl;
        break;
    }
    case SPRM::sprmTTlp:
        tlp.readPtr(ptr);
    ...
    case SPRM::sprmTCellPadding:
        wvlog << "Warning: sprmTCellPadding not implemented" << std::endl;
//         if ( *ptr == 6 ) {
...
//         }
    ...
    case SPRM::sprmTCellSpacingDefault:
        wvlog << "Warning: sprmTCellSpacingDefault not implemented" << std::endl;
        break;
    ...
    case SPRM::sprmTSetShdTable:
        wvlog << "Warning: sprmTSetShdTable not implemented" << std::endl;
        break;
    ...
    case SPRM::sprmPTableProps: {
        ...
        if (dataStream) {
            ...
            wvlog << "Error: no data stream!" << std::endl;
        }
        wvlog << "<-- PrcData parsed successfully" << std::endl;
        break;
    }
    default:
        wvlog << "Huh? None of the defined sprms matches 0x" << std::hex << sprm << std::dec << "... trying to skip anyway" << std::endl;
