...
#ifdef WV2_DEBUG_BOOKMARK
        wvlog << "Num. of bookmarks to start: " << m_start->count() << std::endl;
        wvlog << "m_start init done" << std::endl;
        m_start->dumpCPs();
#endif
    }
...
#ifdef WV2_DEBUG_BOOKMARK
        wvlog << "Num. of bookmarks to end: " << count << std::endl;
        wvlog << "m_end/m_endCP init done" << std::endl;

        if (fib.nFib < Word8nFib) {
            m_end->dumpCPs();
        } else {
            for (uint i = 0; i < m_endCP.size(); i++) {
                wvlog << "dumpCPs:   " << m_endCP[i] << std::endl;
            }
        }
#endif
    }
...
#ifdef WV2_DEBUG_BOOKMARK
    wvlog << "Bookmark init done" << std::endl;
#endif
...
#ifdef WV2_DEBUG_BOOKMARK
        wvlog << "start = " << start << std::endl;
        wvlog << "end = " << end << std::endl;
        wvlog << "name = " << name.ascii() << std::endl;
        wvlog << "valid = " << ok << std::endl;
#endif
